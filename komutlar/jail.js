...
let rol = message.roles.cache.get(data)
if(!rol) return message.channel.send(`Jail rolü ayarlı değil.`)
let yetkili = message.roles.cache.get(data2)
...
if(!kanal) return message.channel.send(`Jail log kanalı ayarlı değil.`)
  ...
  let kişi = message.guild.member(message.mentions.users.first() || message.guild.members.cache(args[0]));
